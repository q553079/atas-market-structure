...
def test_history_bars_with_complete_timezone_stored(tmp_path: Path) -> None:
    ...
    # Verify stored payload preserves all timezone fields
...
def test_history_bars_timezone_source_reflects_atlas_payload(tmp_path: Path) -> None:
...
def test_history_bars_missing_timezone_stores_with_null(tmp_path: Path) -> None:
...
def test_history_bars_stores_native_timeframe_bars(tmp_path: Path) -> None:
    """Only bars matching the payload's bar_timeframe are stored."""
    repo = _create_sqlite_db(tmp_path)
    service = AdapterIngestionService(repository=repo)

    # Payload reports bar_timeframe = "1m" — these are 1-minute bars
    payload_dict = _build_minimal_history_payload(
        symbol="NQH6",
        timeframe="1m",
    )
    ...
    # All 5 bars in the payload should be present
    assert len(obs["bars"]) == 5
    # bar_timeframe in stored payload is exactly "1m" (native)
    assert obs["bar_timeframe"] == "1m"


def test_history_bars_no_higher_timeframe_aggregation_in_payload(tmp_path: Path) -> None:
    ...
    stored = repo.get_ingestion(result.ingestion_id)
    obs = stored.observed_payload
    # Stored bars are exactly the native 1m bars
    assert len(obs["bars"]) == 5
    # None of the stored bars span 5 minutes
    for bar in obs["bars"]:
...
def test_history_bars_duplicate_bar_idempotent_stored(tmp_path: Path) -> None:
    """Two payloads with identical bar data are stored as two separate ingestions.

    The ingestion layer stores every payload; deduplication at query time uses
    the started_at key in candle aggregation (setdefault semantics).
    """
    repo = _create_sqlite_db(tmp_path)
    service = AdapterIngestionService(repository=repo)
    ...
    # Second ingestion — same bars, different message_id (as ATAS would send)
    ...
    # Same bars appear in both ingestions
    assert stored1.observed_payload["bars"] == stored2.observed_payload["bars"]


def test_history_bars_duplicate_bar_candle_aggregation_takes_first(tmp_path: Path) -> None:
    ...
    # Store second payload with same bars
    payload_dict["message_id"] = f"msg-{uuid4().hex[:8]}"
    payload2 = AdapterHistoryBarsPayload.model_validate(payload_dict)
    service.ingest_history_bars(payload2)

    # Both are retrieved by list_ingestions
...
def _create_workbench_repo(tmp_path: Path) -> SQLiteAnalysisRepository:
    db_path = tmp_path / "data" / "market_structure.db"
    db_path.parent.mkdir(parents=True, exist_ok=True)
    connection = sqlite3.connect(str(db_path))
    connection.execute(
...
def test_backfill_request_poll_dispatch_ack_full_chain(tmp_path: Path) -> None:
    """request → poll → dispatch → ingest_history_bars → acknowledge round-trips correctly."""
    repo = _create_workbench_repo(tmp_path)
    ...
    # Step 2: poll — dispatch the pending request
    ...
    # Step 3: ATAS sends back history_bars — ingest them
    t0 = window_start
    bars = [
        {
            "started_at": (t0 + timedelta(minutes=i)).isoformat(),
    ...
    ack_request = AdapterBackfillAcknowledgeRequest(
        ...
        acknowledged_history_bars=[ingest_result.ingestion_id],
        acknowledged_history_footprint=[],
        acknowledged_continuous_state=[],
        acknowledged_continuous_snapshot=None,
    )
    ack_response = workbench.acknowledge_atas_backfill(ack_request)
    assert ack_response.request_id == record.request_id
    assert ack_response.status == ReplayWorkbenchAtasBackfillStatus.ACKNOWLEDGED


def test_backfill_request_reused_on_identical_repeat(tmp_path: Path) -> None:
    ...
    # Same cache_key + instrument + timeframe → reused
