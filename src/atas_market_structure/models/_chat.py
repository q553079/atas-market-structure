...
class ChatMessage(BaseModel):
    ...
    meta: dict[str, Any] = Field(default_factory=dict, description="Additive metadata container for UI-specific projections.")
    created_at: datetime = Field(..., description="Creation timestamp.")
    updated_at: datetime = Field(..., description="Last update timestamp.")
...
class PromptBlock(BaseModel):
    ...
    block_version: int = Field(1, ge=1, description="Additive prompt-block version used for audit-safe trace refs.")
    source_kind: str = Field("system_policy", description="Additive source classification for prompt-block governance.")
    scope: str = Field("request", description="Additive scope classification for prompt-block governance.")
    editable: bool = Field(False, description="Whether the block is editable in attention-first UI flows.")
    author: str | None = Field(None, description="Optional block author/source label.")
    updated_at: datetime | None = Field(None, description="Optional last-updated timestamp for governance metadata.")
